    ...
    for (const line of lines) {
      ...
      try {
        ...
        if (!result._sessionStart && entry.timestamp) {
          result._sessionStart = timestamp;
        }
        ...
        if (entry.type === 'assistant' && entry.message?.usage) {
          ...
          result.reasoningTokens   += u.reasoning_tokens
        ...
        if (entry.type === 'system' && entry.subtype === 'agent_start' && entry.agent_id) {
          agentMap.set(entry.agent_id, {
            id: entry.agent_id, type: 'unknown', status: 'running',
            startTime: timestamp, description: '',
          });
        }
        ...
        if (Array.isArray(content)) {
          for (const block of content) {
            if (block.type === 'tool_use' && block.name) {
              ...
              // Agent invocations
              if (name === 'Agent' || name === 'Task' || name === 'proxy_Task' ||
            ...
            if (block.type === 'tool_result' && block.tool_use_id) {
              ...
              if (agent) {
                ...
                const isBg = typeof blockContent === 'string'
          ...
          if (name === 'agent' || name === 'taskcreate' || name.includes('subagent')) {
            if (agentMap.size < 100) {
              agentMap.set(entry.id || name, {
                id: entry.id, type: 'unknown', status: 'running',
                startTime: timestamp, description: '',
              });
            }
          }
...
function fetchAndCacheRateLimit() {
  ...
  try {
    ...
    const cacheData = {
      ...
      error: false, source: 'anthropic', lastSuccessAt: now,
    };
